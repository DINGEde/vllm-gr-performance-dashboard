#!/usr/bin/env python3
"""Generate dashboard-summary.json for the 2026-08-26 AgentCache Kanban run."""
...
def extract_metrics(summary):
    ...
    metrics = {
        ...
        "vllm_prefix_cache_hit_rate": ratio(
            counters.get("vllm:prefix_cache_hits_total", 0),
            counters.get("vllm:prefix_cache_queries_total", 0),
        ),
        "vllm_prompt_token_hit_rate": ratio(
            counters.get("vllm:prompt_tokens_cached_total", 0),
            counters.get("vllm:prompt_tokens_total", 0),
        ),
        "cli.overrides.max_concurrency": 16,
        "cli.overrides.task_num": 32,
    }
    for name, values in tasks["duration_seconds"].items():
        metrics[f"task_duration_seconds.{name}"] = values
    for name, values in requests["latency_seconds"].items():
        metrics[f"latency_seconds.{name}"] = values
    for name, values in requests["ttft_seconds"].items():
        metrics[f"ttft_seconds.{name}"] = values
    for name, values in vllm["latency_breakdown_seconds"].items():
        metrics[f"vllm_{name}_seconds.mean"] = values["mean"]
    return metrics
...
def main():
    ...
    dashboard = {
        ...
        "provenance": {
            "baseline_cc_result_dir": "results/main-ci-baseline-cc-32_16-20260825_195231",
            "candidate_cc_result_dir": "results/main-ci-candidate-cc-32_16-20260825_195231",
            "baseline_jiuwenswarm_result_dir": "results/main-ci-baseline-jiuwenswarm-32_16-20260825_195231",
            "candidate_jiuwenswarm_result_dir": "results/main-ci-candidate-jiuwenswarm-32_16-20260825_195231",
            "compare_cc_report": "benchkit-logs/main-ci-compare-cc-32_16-20260825_195231.txt",
            "compare_jiuwenswarm_report": "benchkit-logs/main-ci-compare-jiuwenswarm-32_16-20260825_195231.txt",
            "note": "Kanban folder is labeled 2026-08-26. Host launched at 2026-08-25 19:52 CST, so runtime artifact paths carry timestamp 20260825_195231.",
        },
