...
# Commit: 0b5f9b5 (upstream/main, unchanged from 0825; #93 agentbench router-integration refactor)
...
start_candidate() {
  ...
  # #79 replaced the legacy fixed 1800s force-resume timeout with adaptive 30--300s defaults.
  # Pin both bounds to preserve the daily GPU CI behavior validated on L20/Qwen.
  # Keys removed by #79 are deliberately absent; remaining keys were validated at upstream HEAD 0b5f9b5.
  additional_config=$(printf '{"agentcache":{"backend_id":"%s","lifecycle_socket_path":"%s","controller_factory":"agentinfer.agentcache.core.factory.build_progress_ttl_controller","schedule_interval_seconds":%s,"progress_ttl":{"target_max_segment_rounds":14,"resume_capacity_ratio":0.9,"pause_capacity_ratio":0.95,"privileged_max_context_tokens":262144,"paused_program_ttl_seconds":1800,"force_resume_timeout_min_seconds":1800,"force_resume_timeout_max_seconds":1800}}}' "$BACKEND_ID" "$LIFECYCLE_SOCKET" "$SCHEDULE_INTERVAL_SECONDS")
