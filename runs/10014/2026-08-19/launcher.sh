...
# Commit: de56079 (upstream/main, advanced from 2f3da5e; progress-TTL keys #79 removed)
...
start_candidate() {
  ...
  # progress_ttl keys removed by upstream #79 (fd9fcc5): ttl_prefill_seconds_per_1k_uncached_tokens,
  # target_min_segment_rounds, pause_capacity_lookahead_rounds, privileged_lookahead_rounds.
  # Remaining keys are all in _CONFIGURABLE_PROGRESS_TTL_SETTINGS at de56079.
  additional_config=$(printf '{"agentcache":{"backend_id":"%s","lifecycle_socket_path":"%s","controller_factory":"agentinfer.agentcache.core.factory.build_progress_ttl_controller","schedule_interval_seconds":%s,"progress_ttl":{"target_max_segment_rounds":14,"resume_capacity_ratio":0.9,"pause_capacity_ratio":0.95,"privileged_max_context_tokens":262144,"paused_program_ttl_seconds":1800}}}' "$BACKEND_ID" "$LIFECYCLE_SOCKET" "$SCHEDULE_INTERVAL_SECONDS")
