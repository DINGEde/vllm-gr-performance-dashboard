#!/usr/bin/env bash
#
# Launcher: main CI 32/16 baseline + candidate
# Date: 2026-08-06
# Host: L20-10014-direct
# Repo: /home/zhike/dxw/AgentCache
# Commit: 5ab4ac7c194c0a665c2d386c16e60a4369eeda40 (upstream/main)
# Shape: 32/16 (max_concurrency=32, task_num=16)
...
REPO=/home/zhike/dxw/AgentCache
COMMIT=5ab4ac7c194c0a665c2d386c16e60a4369eeda40
...
CANDIDATE_PORT=8400
TP=2
AGENT_BIN=${AGENT_BIN:-/home/zhike/dxw/claude-code/bin/claude}
...
# Hard guards from handoff L210-221
...
# Result directories
BASELINE_RESULT="$REPO/agentinfer/agentbench/results/main-ci-baseline-${SHAPE}-${TIMESTAMP}"
CANDIDATE_RESULT="$REPO/agentinfer/agentbench/results/main-ci-candidate-${SHAPE}-${TIMESTAMP}"
BASELINE_LOG="$LOG_DIR/main-ci-baseline-${SHAPE}-${TIMESTAMP}.log"
CANDIDATE_LOG="$LOG_DIR/main-ci-candidate-${SHAPE}-${TIMESTAMP}.log"
COMPARE_LOG="$LOG_DIR/main-ci-compare-${SHAPE}-${TIMESTAMP}.txt"

echo "Baseline result: $BASELINE_RESULT"
echo "Candidate result: $CANDIDATE_RESULT"
...
# Run baseline
start_baseline
if ! run_arm "$BASELINE_RESULT" "$BASELINE_PORT"; then
  echo "✗ Baseline failed, stopping"
  exit 1
fi
stop_vllm "$VLLM_TMUX_BASELINE"
echo ""
...
# Run candidate
start_candidate
if ! run_arm "$CANDIDATE_RESULT" "$CANDIDATE_PORT"; then
  echo "✗ Candidate failed, stopping"
  exit 1
fi
stop_vllm "$VLLM_TMUX_CANDIDATE"
echo ""
...
if ! grep -q "controller cycle" "$CANDIDATE_LOG"; then
  echo "⚠ WARNING: No controller cycle evidence in candidate log"
fi
echo ""
...
echo "Candidate: $CANDIDATE_RESULT"
echo "Baseline log: $BASELINE_LOG"
echo "Candidate log: $CANDIDATE_LOG"
echo "Compare report: $COMPARE_LOG"
echo ""
echo "Next steps:"
echo "1. Review compare report: cat $COMPARE_LOG"
echo "2. Inspect manifests: cat $BASELINE_RESULT/manifest.json $CANDIDATE_RESULT/manifest.json"
echo "3. Verify candidate scheduler admission/controller activity in: $CANDIDATE_LOG"
