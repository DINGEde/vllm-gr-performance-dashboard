...
def dashboard_markdown(has_runs: bool) -> str:
    ...
    return "\n".join(
        [
            ...
            '  <section class="vgr-section"><div class="vgr-section-head"><div><p class="vgr-kicker">Methodology</p><h2>Metric definitions</h2></div><p>How to read and compare the values.</p></div><div class="vgr-methodology"><p><strong>Canonical Offline E2E hit/miss</strong>: the official daily trend. Each measured pair is <code>reset → measured miss → identical measured hit</code>. The timed call uses one outer monotonic clock and a final CUDA completion fence; output materialization happens after the clock stops.</p><p><strong>Prefill / Decode</strong>: collected in a smaller post-canonical pass after every official E2E sample has finished, with Worker probes disabled. Legacy points use the legacy token-loop host boundary. V1 points are CUDA device-timeline intervals recorded on the single FIFO compute stream: <code>Prefill</code> spans entry into the PREFILL stage through completion of the output-producing Prefill GPU work, and <code>Decode</code> spans entry into the first DECODE stage through the last Decode sample. The Decode start event is stream-ordered behind the Prefill kernels, so it fires at Prefill completion: the two intervals cannot overlap and are additive, and they sum with <strong>Host overhead</strong> (frontend dispatch, EngineCore queueing, and terminal collection) back to E2E. Decode is a device span, so it also absorbs device idle inside the Decode stage. <strong>Prefill dispatch wait</strong> is the host gap from the submit anchor to the Prefill stage entry, <strong>Prefill / Decode CPU lead</strong> is how far the CPU had already advanced into first-Decode preparation when the GPU finished Prefill, and <strong>Prefill output consumed</strong> marks the later EngineCore-visible boundary.</p><p><strong>Pipeline series</strong>: legacy <code>beam_search</code> and V1 <code>beam_search_v1</code> remain on the same metric chart with distinct lines and markers. Lines never connect different pipeline versions.</p><p><strong>Average (Mean)</strong>: arithmetic mean over the relevant canonical or stage observations. The measurement source is shown beside every chart and comparison card.</p><p><strong>Daily trend and PRs</strong>: each date runs only that day\'s latest <code>decode_graph</code> snapshot; the system does not rerun the preceding SHA. Trend points show PRs merged since the preceding published daily snapshot.</p></div></section>',
